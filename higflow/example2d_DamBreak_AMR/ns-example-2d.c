...
// Snapshot the adapted grid without affecting the simulation
// Ensure these includes are at the top of ns-example-2d.c
...
// Interpolate all cell-centred properties in a single loop
...
int main(int argc, char* argv[]) {
    ...
    // =====================================================
    // INITIAL ADAPTATION BASED ON THE ANALYTICAL INTERFACE
    // =====================================================
#if ADAPT_ENABLED
    if (ns->par.step == 0) {
        int order_center = 1;
    ...
    for (; ns->par.step <= ns->par.finalstep; ns->par.step++) {
        ...
        if (ns->par.step % ADAPT_FREQ == 0) {
              ...
              // Inherit parameters and controllers from the old solver (avoids YAML re-read)
              ns2->par = ns->par;
              ns2->contr = ns->contr;
              ns2->sdp = ns->sdp;
              ns2->sdF = ns->sdF;
              ...
              // 2. Copy essential parameters from the old solver
              ns2->par = ns->par;
              ns2->contr = ns->contr;
              higflow_destroy(ns);
              ...
              // Good practice to sync pressure as well
