...
/*!
 * \brief Level-set function for the dam-break water column.
 *
 * Signed distance to a rectangle [0,1.2] × [0,0.602].
 * Negative inside the column (phase 0 = water), positive outside.
 * The top coordinate 0.602 avoids alignment with cell faces
 * on the 160×100 mesh (dy = 0.02 → 0.602/0.02 = 30.1, non-integer).
 */
real
func(Point p)
{
    real xmin = 0.0, xmax = 1.2;
    real ymin = 0.0, ymax = 0.602;

    if (p[0] >= xmin && p[0] <= xmax && p[1] >= ymin && p[1] <= ymax)
    {
        real dx_in = fmin(p[0] - xmin, xmax - p[0]);
        real dy_in = fmin(p[1] - ymin, ymax - p[1]);
        return -fmin(dx_in, dy_in);
    }
