...
// =========================================================================================
// FUNÇÃO DE ADAPTAÇÃO
// thresholds: array terminado em sentinela -1.0, num_levels calculado internamente
// =========================================================================================
hig_cell * higflow_make_adapted_tree_params(higflow_solver *ns, real *thresholds) {
    ...
    while (!higcit_isfinished(it)) {
        ...
        if (clid >= 0) {
            real val;
            if (ns->par.step == 0) {
                // Usa interface analítica na primeira adaptação
                Point xc, delta;
                hig_get_center(c, xc);
...
void higflow_refine_tree_inplace(higflow_solver *ns, real *thresholds) {
    ...
    if (seed_count > 0) {
        for (int pass = 0; pass < num_levels + 1; pass++) {
            ...
            for (int s = 0; s < seed_count; s++) {
                ...
                higcit_celliterator *it_bb = higcit_create_bounding_box(root, lo, hi);
                while (!higcit_isfinished(it_bb)) {
                    hig_cell *neigh = higcit_getcell(it_bb);
                    if (hig_get_number_of_children(neigh) == 0) {
                        ...
                        if (current_lvl < target_level) {
                            ...
                            int added = 0;
                            for (int k = 0; k < refine_count; k++) if (to_refine[k] == neigh) { added = 1; break; }
                            if (!added) to_refine[refine_count++] = neigh;
                        }
                    }
                    higcit_nextcell(it_bb);
                }
            ...
            if (refine_count == 0 && pass > 0) { if (pass > num_levels) break; }
            for (int i = 0; i < refine_count; i++) {
                if (hig_get_number_of_children(to_refine[i]) == 0) {
                    int nc[DIM];
                    for (int d = 0; d < DIM; d++) nc[d] = 2;
    ...
    // ETAPA B: ENGROSSAMENTO in-place (4 Passes)
    float coarsen_hys = 0.005;
    for (int pass = 0; pass < 4; pass++) {
        int merge_cap = 1000;
        int merge_count = 0;
        hig_cell **parents_to_merge = (hig_cell **)malloc(merge_cap * sizeof(hig_cell*));
                ...
                if (all_leaves) {
                    ...
                    if (seed_count > 0) {
                        for (int k = 0; k < hig_get_number_of_children(c); k++) {
                            ...
                            real dist = sqrt(min_d2);
                            real threshold = (parent_lvl < num_levels) ? thresholds[parent_lvl] + coarsen_hys : -1.0;
                            if (dist <= threshold) { safe_to_merge = 0; break; }
                        }
                    }
                    if (safe_to_merge) {
                        if (merge_count >= merge_cap) {
        ...
        higcit_destroy(it_all);
        if (merge_count > 0) {
            for (int i = 0; i < merge_count; i++) hig_merge_children(parents_to_merge[i]);
        } else { break; }
        free(parents_to_merge);
    }
    free(seeds);
