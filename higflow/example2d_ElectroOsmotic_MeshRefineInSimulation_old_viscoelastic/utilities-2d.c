...
void higflow_interpolate_viscosity(higflow_solver *ns, higflow_solver *ns2) {
    ...
    for (it = sd_get_domain_celliterator(sdm2); !higcit_isfinished(it); higcit_nextcell(it)) {
        ...
        // Note: Using 'sdm' (old domain) and 'ns->stn' (old stencil)
        real fracvol = compute_value_at_point(sdm, ccenter, ccenter, 1.0, ns->ed.mult.dpfracvol, ns->ed.mult.stn);
        
        // Calculate the viscosity based on the interpolated fraction
        ...
        // É importante atualizar também o fracvol no novo solver, pois ele é a base da física
...
void higflow_interpolate_density(higflow_solver *ns, higflow_solver *ns2) {
    ...
    for (it = sd_get_domain_celliterator(sdm2); !higcit_isfinished(it); higcit_nextcell(it)) {
        ...
        // Interpolate fracvol from the OLD solver (ns)
        // Usamos o domínio 'sdm' (velho) e a propriedade 'dpfracvol' do 'ns' (velho)
        real fracvol = compute_value_at_point(sdm, ccenter, ccenter, 1.0, ns->ed.mult.dpfracvol, ns->ed.mult.stn);

        // Calculate the density using the user functions (typically spatially varying or constant)
        // Usamos o tempo atual (ns->par.t) e o centro geométrico
        real dens0 = ns->ed.mult.get_density0(ccenter, ns->par.t);
        real dens1 = ns->ed.mult.get_density1(ccenter, ns->par.t);
        
        // Mix density based on interpolated volume fraction
        real dens = (1.0 - fracvol) * dens0 + fracvol * dens1;

        // Set the density in the distributed density property of the NEW solver (ns2)
    ...
    // Sync the distributed density property in the new solver
    dp_sync(ns2->ed.mult.dpdens);
}

// void higflow_interpolate_polymeric_tensor(higflow_solver *ns, higflow_solver *ns2) {
//     // Verifica se a simulação é viscoelástica antes de tentar acessar memória
//     if (ns->ed.mult.contr.viscoelastic_either == false) return;
//
//     // Obtém domínios
//     sim_domain *sdm = psd_get_local_domain(ns->ed.mult.psdmult);   // Velho (Fonte)
//     sim_domain *sdm2 = psd_get_local_domain(ns2->ed.mult.psdmult); // Novo (Destino)
//
//     // Mapper do novo domínio
//     mp_mapper *mp2 = sd_get_domain_mapper(sdm2);
//
//     // O Tensor é uma propriedade distribuída para cada componente (dim x dim)
//     for (int i = 0; i < DIM; i++) {
//         for (int j = 0; j < DIM; j++) {
//
//             // Loop sobre as células da NOVA malha
//             higcit_celliterator *it;
//             for (it = sd_get_domain_celliterator(sdm2); !higcit_isfinished(it); higcit_nextcell(it)) {
//                 hig_cell *c = higcit_getcell(it);
//                 int clid = mp_lookup(mp2, hig_get_cid(c));
...
//                 hig_get_center(c, ccenter);
//
//                 // Interpola o componente [i][j] do tensor da malha VELHA
//                 // ns->ed.mult.dpS[i][j] guarda o tensor na estrutura multifásica
//                 real val_tensor;
//
//                 // Reset e Stencil no domínio velho
//                 stn_reset(ns->stn);
//                 sd_get_stencil(sdm, ccenter, ccenter, 1.0, ns->stn); // 1.0 = ordem linear
//
//                 val_tensor = compute_value_at_point(sdm, ccenter, ccenter, 1.0, 
//                                                     ns->ed.mult.dpS[i][j], ns->stn);
//
//                 // Seta o valor na malha NOVA
//                 dp_set_value(ns2->ed.mult.dpS[i][j], clid, val_tensor);
//             }
//             higcit_destroy(it);
//
//             // Sincroniza o componente atual
//             dp_sync(ns2->ed.mult.dpS[i][j]);
//         }
//     }
// }
