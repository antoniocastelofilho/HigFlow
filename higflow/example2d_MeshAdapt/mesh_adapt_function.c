...
#define MAX_REF_LEVEL 2
#define SEARCH_DIST 0.04
#define SEARCH_DIST_TRANSITION 0.05
...
void higflow_save_refined_mesh_preview(higflow_solver *ns, int frame_id) {
    ...
    float search_dist[2]; // Tamanho fixo, mas usando a constante em cálculos
    search_dist[0] = SEARCH_DIST;
    search_dist[1] = SEARCH_DIST_TRANSITION;

    float refiment_levels[2]; // Tamanho fixo, mas usando a constante em cálculos
    refiment_levels[0] = MAX_REF_LEVEL;
    refiment_levels[1] = MAX_REF_LEVEL-1;
    ...
    for (int pass = 0; pass < MAX_REF_LEVEL; pass++) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            for(int d=0; d<DIM; d++) numcells[d] = pow(2, refiment_levels[pass]); 
