...
// 2D rising drop (cap regime) WITH adaptive mesh refinement around the
// interface.  Same physics as example2d_RisingDrop; AMR machinery adapted
// from example3d_RisingDropCap.  AMR runs serial only (NP=1).
...
// Mixing rule value = (1 - fracvol)*x0 + fracvol*x1 ; drop (fracvol==1) = phase 1.
...
/* One refinement level near the interface (|fracvol gradient| zone within
 * 0.04 of an interface cell).  Keeps dx_min ~ 0.0156 on the 32x64 base, i.e.
 * effective 64x128 around the drop, which is stable at dt=5e-4. */
real REFINE_THRESHOLDS[] = {0.04, -1.0};
#include "mesh_adapt_function.c"

// Interpolate cell-centred fields (fracvol, viscosity, density, pressure)
// from the old solver onto the newly adapted one.
void higflow_interpolate_all_cells_2d(higflow_solver *ns, higflow_solver *ns2) {
    ...
    for (it = sd_get_domain_celliterator(sdm2); !higcit_isfinished(it); higcit_nextcell(it)) {
        ...
        // Re-sharpen the interpolated fraction (interpolation across the
        // coarse-fine boundary smears it; clip near-pure cells back to 0/1).
        real clip = 0.4;
        if (fracvol >= (1.0 - clip))      fracvol = 1.0;
        else if (fracvol <= clip)         fracvol = 0.0;
...
// Interpolate face-centred velocity from the old solver onto the new one.
void higflow_interpolate_velocity_2d(higflow_solver *ns, higflow_solver *ns2) {
...
// Interpolate Dirichlet pressure BC values from the old solver to the new one.
void higflow_interpolate_all_bcs_2d(higflow_solver *ns, higflow_solver *ns2) {
    sim_domain *sdp  = psd_get_local_domain(ns->psdp);
    sim_domain *sdp2 = psd_get_local_domain(ns2->psdp);
    for (int i = 0; i < 2; i++) {
        bc_type bc_t = (i == 0) ? DIRICHLET : NEUMANN;
...
int main(int argc, char *argv[]) {
    ...
    for (int step0 = ns->par.initstep; ns->par.step <= ns->par.finalstep; ns->par.step++) {
        ...
        if (ns->par.step % ADAPT_FREQ == 0) {
            ...
            higflow_interpolate_velocity_2d(ns, ns2);
            higflow_interpolate_all_cells_2d(ns, ns2);
            // Rebuild interface geometry on the new mesh before stepping,
            // otherwise the first step uses stale PLIC/curvature and blows up.
            ...
            higflow_interpolate_all_bcs_2d(ns, ns2);
