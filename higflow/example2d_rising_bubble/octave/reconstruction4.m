...
function[ ]=calculation_normal_curvature(Xh,Yh,delta,X,Y)
	...
	for I=1:Nx
		for J=1:Ny
			...
			[H_middle auxh origh]=horizontal_row	(I,J,Xh,Yh,'MIDDLE');
			[H_top auxh_t origh_t]=horizontal_row(I,J+1,Xh,Yh,'TOP');
			[H_bottom auxh_b origh_b]=horizontal_row(I,J-1,Xh,Yh,'BOTTOM');
			if(abs(auxh+auxh_t+auxh_b)==3)
				H=set_common_orig_horizontal([H_middle,H_top,H_bottom],...
				auxh,[origh,origh_t,origh_b],J,Yh,dx);
				H_middle=H(1);H_top=H(2);H_bottom=H(3);
				calculation_normal_cell_central_2nd_order_finite_difference_Horizontal(H_bottom,H_middle,H_top,dx,dy,I,J,auxh)
				calculation_curvature_cell_central_2nd_order_finite_difference_Horizontal(H_bottom,H_middle,H_top,dx,dy,I,J,auxh)
				NormalAux(I,J)=2;%Naux
				continue;
			else
				[V_middle auxv origv]=vertical_collumn(I,J,Xh,Yh,'MIDDLE');
				[V_right auxv_r origv_r]=vertical_collumn(I+1,J,Xh,Yh,'RIGHT');
				[V_left auxv_l origv_l]=vertical_collumn(I-1,J,Xh,Yh,'LEFT');
				if(abs(auxv+auxv_l+auxv_r)==3)
					V = set_common_orig_vertical([V_middle,V_right,V_left],auxv,[origv,origv_r,origv_l],I,Xh,dy);
					V_middle=V(1);V_right=V(2);V_left=V(3);
					calculation_normal_cell_central_2nd_order_finite_difference_Vertical(V_left,V_middle,V_right,dx,dy,I,J,auxv);
					calculation_curvature_cell_central_2nd_order_finite_difference_Vertical(V_left,V_middle,V_right,dx,dy,I,J,auxv);
					NormalAux(I,J)=1;%Naux
					continue;
				end
			end
			if(abs(auxh+auxh_t)==2)
				[H_top_top auxh_t_t origh_t_t]=horizontal_row(I,J+2,Xh,Yh,'g');
				if(abs(auxh+auxh_t+auxh_t_t)==3)
					H=set_common_orig_horizontal([H_middle,	H_top,H_top_top],auxh,[origh,origh_t,origh_t_t],J,Yh,dx);
					H_middle=H(1);H_top=H(2);H_top_top=H(3);
					calculation_normal_cell_progressive_2nd_order_finite_difference_Horizontal(H_middle,H_top,H_top_top,dx,dy,I,J,auxh);
					calculation_curvature_cell_progressive_1st_order_finite_difference_Horizontal(H_middle,H_top,H_top_top,dx,dy,I,J,auxh);
					NormalAux(I,J)=2;%Naux
					continue;
				end
			end
			if(abs(auxh+auxh_b)==2)
				[H_bottom_bottom auxh_b_b origh_b_b]=horizontal_row(I,J-2,Xh,Yh,'g');
				if(abs(auxh+auxh_b+auxh_b_b)==3)
					H=set_common_orig_horizontal([H_middle,H_bottom,H_bottom_bottom],auxh,[origh,origh_b,origh_b_b],J,Yh,dx);
					H_middle=H(1);H_bottom=H(2);H_bottom_bottom=H(3);
					calculation_normal_cell_regressive_2nd_order_finite_difference_Horizontal(H_bottom_bottom,H_bottom,H_middle,dx,dy,I,J,auxh);
					calculation_curvature_cell_regressive_1st_order_finite_difference_Horizontal(H_bottom_bottom,H_bottom,H_middle,dx,dy,I,J,auxh);
					NormalAux(I,J)=2;%Naux
					continue;
				end
			end
