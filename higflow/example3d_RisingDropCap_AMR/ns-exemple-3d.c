...
real get_density0(Point center, real t) { return 0.001; }
real get_density1(Point center, real t) { return 1.0; }
real get_viscosity0(Point center, real t) { return 0.01; }
real get_viscosity1(Point center, real t) { return 1.0; }
...
/*!
 * \brief Run the simulation and log phase timing.
 *
 * Prints timestamps so we can identify where time is spent.
 */
int main(int argc, char *argv[]) {
    ...
    if (ns->par.step == 0) {
        ...
        ns2->ed.mult.contr = ns->ed.mult.contr;
        ns = ns2;
    ...
    /* Print VTK for the initial (adapted) mesh — needs solver for all fields */
    if (ns->par.step == 0) {
