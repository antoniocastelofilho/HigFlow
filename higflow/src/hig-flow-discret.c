					...
					else
					{
						...
						else
						{
							...
							else
							{
								if (fi < b)
									if (infacet_rr == 1)
										conv1 = ns->cc.vc[dim2] * (a * ur[dim] - c * urr[dim]);
									else
										conv1 = ns->cc.vc[dim2] * ur[dim];
								if ((fi >= b) && (fi <= c))
									if (infacet_rr == 1)
										conv1 = ns->cc.vc[dim2] * (c * ur[dim] + b * ns->cc.ufacet - d * urr[dim]);
									else
										conv1 = ns->cc.vc[dim2] * ur[dim];
								if (fi > c)
					...
					if (ns->cc.vc[dim2] > 0.0)
					{
						...
						else
						{
							...
							else
							{
								if (fi < b)
									if (infacet_ll == 1)
										conv2 = ns->cc.vc[dim2] * (a * ul[dim] - c * ull[dim]);
									else
										conv2 = ns->cc.vc[dim2] * ul[dim];
								if ((fi >= b) && (fi <= c))
									if (infacet_ll == 1)
										conv2 = ns->cc.vc[dim2] * (b * ns->cc.ufacet + c * ul[dim] - d * ull[dim]);
									else
										conv2 = ns->cc.vc[dim2] * ul[dim];
								if (fi > c)
	...
	case ORDER2:
		...
		for (int dim2 = 0; dim2 < DIM; dim2++)
		{
			...
			else
			{
				...
				POINT_ASSIGN(p1, fcenter);
				POINT_ASSIGN(p2, fcenter);

				p1[dim] = p1[dim] - 0.5 * fdelta[dim];
				p2[dim] = p2[dim] + 0.5 * fdelta[dim];
				...
				p3[dim2] = p3[dim2] + fdelta[dim2];
				p4[dim2] = p4[dim2] + fdelta[dim2];
				p3_[dim2] = p3_[dim2] - fdelta[dim2];
				p4_[dim2] = p4_[dim2] - fdelta[dim2];
