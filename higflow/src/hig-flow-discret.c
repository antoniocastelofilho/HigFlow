   ...
   case ORDER2:
      ...
      for (int dim2 = 0; dim2 < DIM; dim2++) {
         ...
         if(dim2==dim){
            ...
            real v1=compute_value_at_point(ns->ed.sdED,fcenter,p1,1.0,ns->ed.gn.dpvisc,ns->ed.stn);
            real v2=compute_value_at_point(ns->ed.sdED,fcenter,p2,1.0,ns->ed.gn.dpvisc,ns->ed.stn);
            
            real v3=compute_value_at_point(ns->ed.sdED,fcenter,p3,1.0,ns->ed.gn.dpvisc,ns->ed.stn);
            real v4=compute_value_at_point(ns->ed.sdED,fcenter,p4,1.0,ns->ed.gn.dpvisc,ns->ed.stn);
            
            real v3_=compute_value_at_point(ns->ed.sdED,fcenter,p3_,1.0,ns->ed.gn.dpvisc,ns->ed.stn);
            real v4_=compute_value_at_point(ns->ed.sdED,fcenter,p4_,1.0,ns->ed.gn.dpvisc,ns->ed.stn);
      ...
      for (int dim2 = 0; dim2 < DIM; dim2++) {
         ...
         if(ns->ed.mult.contr.viscoelastic_either == true) {
            ...
            if(dim2==dim) {
               ...
               real visc_sl0 = beta0 * ns->ed.mult.problem->viscosity0(pl, ns->par.t);
               real visc_sl1 = beta1 * ns->ed.mult.problem->viscosity1(pl, ns->par.t);
               ...
               real visc_sr0 = beta0 * ns->ed.mult.problem->viscosity0(pr, ns->par.t);
               real visc_sr1 = beta1 * ns->ed.mult.problem->viscosity1(pr, ns->par.t);
               ...
               // viscosity 
               real vs10 = beta0 * ns->ed.mult.problem->viscosity0(p1, ns->par.t);
               real vs11 = beta1 * ns->ed.mult.problem->viscosity1(p1, ns->par.t);
               ...
               real vs20 = beta0 * ns->ed.mult.problem->viscosity0(p2, ns->par.t);
               real vs21 = beta1 * ns->ed.mult.problem->viscosity1(p2, ns->par.t);
               ...
               real vs30 = beta0 * ns->ed.mult.problem->viscosity0(p3, ns->par.t);
               real vs31 = beta1 * ns->ed.mult.problem->viscosity1(p3, ns->par.t);
               ...
               real vs40 = beta0 * ns->ed.mult.problem->viscosity0(p4, ns->par.t);
               real vs41 = beta1 * ns->ed.mult.problem->viscosity1(p4, ns->par.t);
               ...
               real vs3_0 = beta0 * ns->ed.mult.problem->viscosity0(p3_, ns->par.t);
               real vs3_1 = beta1 * ns->ed.mult.problem->viscosity1(p3_, ns->par.t);
               ...
               real vs4_0 = beta0 * ns->ed.mult.problem->viscosity0(p4_, ns->par.t);
               real vs4_1 = beta1 * ns->ed.mult.problem->viscosity1(p4_, ns->par.t);
            ...
            // NO BSD
            // ns->cc.viscl[dim2] = visc_sl;
            // ns->cc.viscr[dim2] = visc_sr;
...
void higflow_computational_cell_multiphase_electroosmotic_ionic(higflow_solver *ns, sim_domain *sdn, int clid, Point ccenter, Point cdelta, int dim, distributed_property *dpn, sim_stencil *stnn,
   ...
   real psic, phic, psir,psil, phir, phil, nc, nr, nl, ul, ur;
   real alphaeolc, alphaeorc, Pelc, Perc;
   int  infacet_l, infacet_r;
   switch (ns->contr.spatialdiscrtype) {
   // Second order
   case ORDER2:
      ...
      nl    = compute_center_p_left(sdn, ccenter, cdelta, dim, 1.0, dpn, stnn);
      nr    = compute_center_p_right(sdn, ccenter, cdelta, dim, 1.0, dpn, stnn);
      psil  = compute_center_p_left(ns->ed.eo.sdEOpsi, ccenter, cdelta, dim, 1.0, ns->ed.eo.dppsi, ns->ed.eo.stnpsi);
      psir  = compute_center_p_right(ns->ed.eo.sdEOpsi, ccenter, cdelta, dim, 1.0, ns->ed.eo.dppsi, ns->ed.eo.stnpsi);
      phil  = compute_center_p_left(ns->ed.eo.sdEOphi, ccenter, cdelta, dim, 1.0, ns->ed.eo.dpphi, ns->ed.eo.stnphi);
      phir  = compute_center_p_right(ns->ed.eo.sdEOphi, ccenter, cdelta, dim, 1.0, ns->ed.eo.dpphi, ns->ed.eo.stnphi);
      ...
      // Extra terms that consider non-uniform properties at the interface
      // div(1/Pe*grad(n))
      Pelc = 0.5*(Pel + Pe); Perc = 0.5*(Per + Pe);
      ns->cc.d2ndx2  = (nl/Pelc - nc/(Pelc + Perc) + nr/Perc)/(cdelta[dim]*cdelta[dim]);
      ns->cc.d2ndx2 /= 1.0/Pe; // is gonna be multiplied in terms

      // div(a/Pe*grad(Phi))
      alphaeolc = 0.5*(alphaeol + alphaeo); alphaeorc = 0.5*(alphaeor + alphaeo);
      ns->cc.d2psidx2 = (psil*alphaeolc/Pelc - psic*(alphaeolc/Pelc + alphaeorc/Perc) + psir*alphaeorc/Perc)/(cdelta[dim]*cdelta[dim]);
      ns->cc.d2psidx2 /= alphaeo/Pe; // is gonna be multiplied in terms
      ns->cc.d2phidx2 = (phil*alphaeolc/Pelc - phic*(alphaeolc/Pelc + alphaeorc/Perc) + phir*alphaeorc/Perc)/(cdelta[dim]*cdelta[dim]);
      ns->cc.d2phidx2 /= alphaeo/Pe; // is gonna be multiplied in terms
      break;
   }
}
