...
void higflow_print_vtk2D(higflow_solver *ns, int rank) {
    ...
    for(it = sd_get_domain_celliterator(sdp); !higcit_isfinished(it); higcit_nextcell(it)) {
        ...
        // Pontos onde será interpolada a velocidade
        Point p0, p1;
        p0[0] = ccenter[0] - 0.5*cdelta[0];
        p0[1] = ccenter[1];
        p1[0] = ccenter[0] + 0.5*cdelta[0];
        p1[1] = ccenter[1];

        real value0 = compute_facet_value_at_point(sfdu2[0], ccenter, p0, 1.0, ns->dpu[0], ns->stn);
        real value1 = compute_facet_value_at_point(sfdu2[0], ccenter, p1, 1.0, ns->dpu[0], ns->stn);
        real val    = 0.5*(value0+value1);
        fprintf(f, "%f\n", value0);
    }
    higcit_destroy(it);
    fclose(f);
...
void higflow_load_data_files(int argc, char *argv[], higflow_solver *ns) {
    // Name of load file
    ns->par.nameload = argv[1]; argv++;
    // Name of save file
    ns->par.namesave = argv[1]; argv++;
    // Name of print file
    ns->par.nameprint = argv[1]; argv++;
}
