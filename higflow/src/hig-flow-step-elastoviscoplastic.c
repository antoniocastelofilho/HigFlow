...
void higflow_compute_kernel_tensor_elastoviscoplastic(higflow_solver *ns) {
    if ((ns->contr.flowtype == 8)){
        // Get the cosntants
        real Re   = ns->par.Re;
        real De   = ns->ed.vepl.par.De;
        real beta = ns->ed.vepl.par.beta;
        real tol  = ns->ed.vepl.par.kernel_tol;
        //tol       = 1.0e-3;
        ...
        for (it = sd_get_domain_celliterator(sdp); !higcit_isfinished(it); higcit_nextcell(it)) {
            ...
            for (int i = 0; i < DIM; i++) {
                for (int j = 0; j < DIM; j++) {
                    // Get Du
                    Du[i][j] = compute_value_at_point(ns->ed.sdED, ccenter, ccenter, 1.0, ns->ed.vepl.dpDu[i][j], ns->ed.stn);
                    // Get S
                    S[i][j]  = compute_value_at_point(ns->ed.sdED, ccenter, ccenter, 1.0, ns->ed.vepl.dpS[i][j], ns->ed.stn);
                }
            }
            ...
            for (int i = 0; i < DIM; i++) {
                for (int j = 0; j < DIM; j++) {
                   dp_set_value(ns->ed.vepl.dpKernel[i][j], clid, Kernel[i][j]);
                }
            }
        }
        //printf("PAUSE 72\n");getchar();
        ...
        for (int i = 0; i < DIM; i++) {
            for (int j = 0; j < DIM; j++) {
                dp_sync(ns->ed.vepl.dpKernel[i][j]);
            }
        }
    }
}
        ...
        for (it = sd_get_domain_celliterator(sdp); !higcit_isfinished(it); higcit_nextcell(it)) {
            ...
            for (int i = 0; i < DIM; i++) {
                for (int j = 0; j < DIM; j++) {
                    ...
                    Du[i][j] = compute_value_at_point(ns->ed.sdED, ccenter, ccenter, 1.0, ns->ed.vepl.dpDu[i][j], ns->ed.stn);
                    // Get Kernel
                    Kernel[i][j] = compute_value_at_point(ns->ed.sdED, ccenter, ccenter, 1.0, ns->ed.vepl.dpKernel[i][j], ns->ed.stn);
                }
            }
            ...
            for (int i = 0; i < DIM; i++) {
                for (int j = 0; j < DIM; j++) {
                   ...
                   dp_set_value(ns->ed.vepl.dpS[i][j], clid, S[i][j]);
                }
            }
        }
        for (int i = 0; i < DIM; i++) {
...
void higflow_explicit_euler_constitutive_equation_elastoviscoplastic(higflow_solver *ns) {
    if ((ns->contr.flowtype == 8)) {
        ...
        real beta  = ns->ed.vepl.par.beta;
        real tol   = ns->ed.vepl.par.kernel_tol;
        real zeta  = ns->ed.vepl.par.zeta;
        real Bi    = ns->ed.vepl.par.Bi;
        real Np    = ns->ed.vepl.par.Np;
        real epsilon = ns->ed.vepl.par.epsilon;
        real small = 1.0e-14;
        real smallTD = 1.0e-16;
        ...
        for (it = sd_get_domain_celliterator(sdp); !higcit_isfinished(it); higcit_nextcell(it)) {
            ...
            for (int i = 0; i < DIM; i++) {
                ...
                //tr += S[i][i];
                //tr += S[i][i] + 2.0*(1.0-beta)*D[i][i]/Re;
            }
            // Calculate the tensor A
            real A[DIM][DIM], D[DIM][DIM];
            for (int i = 0; i < DIM; i++) {
                ...
                A[i][i] += 1.0;
                //Trace of the elastic stress tensor T
                tr += S[i][i] + 2.0*(1.0-beta)*D[i][i]/Re;
            }

            //Calculate TD (deviatoric stress tensor in terms of S)
            real TD[DIM][DIM];
            real DIM2E;
            if (DIM == 2)
            DIM2E = 2.0;
            ...
            for (int i = 0; i < DIM; i++) {
                for (int j = 0; j < DIM; j++) {
                        TD[i][j] = S[i][j] + 2.0*(1.0-beta)*D[i][j]/Re;
						//TD[i][j] = S[i][j];
                }
                TD[i][i] -= tr/DIM2E;
                //TD[i][i] -= tr/2.0;
            ...
            // Calculate the double dot product of the deviatoric stress tensor TD
            real TD2[DIM][DIM];
            for (int dim = 0; dim < DIM; dim++) {
                for (int dim2 = 0; dim2 < DIM; dim2++) {
                    TD2[dim][dim2] = 0.0;
                    for (int dim3 = 0; dim3 < DIM; dim3++) {
                        TD2[dim][dim2] += TD[dim][dim3] * TD[dim3][dim2];
                    }
                }
            }
            // Calculate sigma_D 
            ...
            for (int i = 0; i < DIM; i++) {
                for (int j = i; j < DIM; j++) {
                    ...
                    // Store Kernel in S
                    dp_set_value(ns->ed.vepl.dpS[i][j], clid, kernel);
                    if (i != j) {
                         dp_set_value(ns->ed.vepl.dpS[j][i], clid, kernel);
                    }
        ...
        for (it = sd_get_domain_celliterator(sdp); !higcit_isfinished(it); higcit_nextcell(it)) {
            ...
            for (int i = 0; i < DIM; i++) {
                for (int j = 0; j < DIM; j++) {
                    ...
                    S[i][j]  = compute_value_at_point(ns->ed.sdED, ccenter, ccenter, 1.0, ns->ed.vepl.dpS[i][j], ns->ed.stn);
                    // Store Kernel
                    dp_set_value(ns->ed.vepl.dpKernel[i][j], clid, S[i][j]);
                }
            }
        }
...
real hig_flow_convective_tensor_term_cubista(higflow_solver *ns, distributed_property *dpu, sim_domain *sdp, sim_stencil *stn, real K[DIM][DIM], Point ccenter, Point cdelta, int dim, int i, int j) {
    ...
    kl  = compute_center_p_left_22(ns->ed.sdED, ccenter, cdelta, dim, 1.0, ns->ed.vepl.dpKernel[i][j], ns->ed.stn, &incell_l); 
    kr  = compute_center_p_right_22(ns->ed.sdED, ccenter, cdelta, dim, 1.0, ns->ed.vepl.dpKernel[i][j], ns->ed.stn, &incell_r); 
    kll = compute_center_p_left_22(ns->ed.sdED, ccenter, cdelta, dim, 2.0, ns->ed.vepl.dpKernel[i][j], ns->ed.stn, &incell_ll);
    krr = compute_center_p_right_22(ns->ed.sdED, ccenter, cdelta, dim, 2.0, ns->ed.vepl.dpKernel[i][j], ns->ed.stn, &incell_rr);
...
// *******************************************************************
void higflow_explicit_euler_intermediate_velocity_elastoviscoplastic(higflow_solver *ns, distributed_property *dpu[DIM], distributed_property *dpustar[DIM]) {
...
void higflow_explicit_runge_kutta_2_intermediate_velocity_elastoviscoplastic(higflow_solver *ns) {
    ...
    higflow_explicit_euler_intermediate_velocity_elastoviscoplastic(ns, ns->dpu, ns->dpuaux);
    // Calculate the star velocity by the explicit euler method
    higflow_explicit_euler_intermediate_velocity_elastoviscoplastic(ns, ns->dpuaux, ns->dpustar);
...
// *******************************************************************
void higflow_semi_implicit_bdf2_intermediate_velocity_elastoviscoplastic(higflow_solver *ns, distributed_property *dpu[DIM], distributed_property *dpustar[DIM]) {
...
void higflow_solver_step_elastoviscoplastic(higflow_solver *ns) {
    ...
    // Computing the Polymeric Tensor
    higflow_compute_polymeric_tensor_elastoviscoplastic(ns);
}
...
void hig_flow_calculate_m_oldroyd_bingham (higflow_solver *ns, real lambda[DIM], real M[DIM][DIM], real R[DIM][DIM], real M_aux[DIM][DIM], real tol, real smallTD, real SD) {
    ...
    //Calculate the function f(T) = max(0,(SD-bi)/SD)
    real FBI = (SD-ns->ed.vepl.par.Bi)/(SD + smallTD);
    real zero = 0.0;
    real F_SD = fmax(zero, FBI);
    ...
    for (int i = 0; i < DIM; i++) {
        for (int j = 0; j < DIM; j++) {
            M_aux[i][j] += -2.0*(B[i][j]*lambda[j]-B[i][j])*ns->ed.vepl.par.De*ns->ed.vepl.par.zeta*jlambda[j];
        }
    }
...
void hig_flow_calculate_m_oldroyd_HB (higflow_solver *ns, real lambda[DIM], real M[DIM][DIM], real R[DIM][DIM], real M_aux[DIM][DIM], real tol, real smallTD, real SD) {
    ...
    //Calculate the function f(T) = max( 0,(SD-bi)/ (coef*(SD)^n) )^(1/n)
    double Np = ns->ed.vepl.par.Np;
    double BRE = (1.0-ns->ed.vepl.par.beta)/(ns->par.Re);
    double n1 = 1.0 - Np;
    double c1 = 1.0/pow(BRE,n1);
    double SDN = pow(SD,Np);
    double FBI = (SD-ns->ed.vepl.par.Bi)/(c1*SDN + smallTD);
    double zero = 0.0;
    double F_SDA = fmax(zero, FBI);
    double NP1 = 1.0/Np;
    double F_SD = pow(F_SDA,NP1);
    // Calculate the MM matrix
    for (int i = 0; i < DIM; i++) {
        for (int j = i+1; j < DIM; j++) {
            M_aux[i][j] = 0.0;
    ...
    for (int i = 0; i < DIM; i++) {
        for (int j = 0; j < DIM; j++) {
            M_aux[i][j] += -2.0*(B[i][j]-B[i][j]*lambda[j])*ns->ed.vepl.par.De*ns->ed.vepl.par.zeta*jlambda[j];
        }
    }
...
void hig_flow_calculate_m_lptt_bingham (higflow_solver *ns, real tr, real lambda[DIM],  real M[DIM][DIM], real R[DIM][DIM], real M_aux[DIM][DIM], real tol, real smallTD, real SD) {
    ...
    hig_flow_matrix_transpose_product(B_aux, R, B);
    //Calculate the function f(T) = max(0,(SD-bi)/SD)
    double FBI = (SD-ns->ed.vepl.par.Bi)/(SD +smallTD);
    double zero = 0.0;
    double F_SD = fmax(zero, FBI);
    // Calculate the MM matrix
    for (int i = 0; i < DIM; i++) {
        for (int j = i+1; j < DIM; j++) {
            M_aux[i][j] = 0.0;
...
// Calculate the matrix MM for GPTT-Bingham model
...
void hig_flow_derivative_kernel_at_center_cell (higflow_solver *ns, Point ccenter, Point cdelta, int i, int j, real Kcenter, real dKdx[DIM]) {
    for (int dim = 0; dim < DIM; dim++) {
        ...
        real Kleft = compute_center_p_left_22(ns->ed.sdED, ccenter, cdelta, dim, 1.0, ns->ed.vepl.dpKernel[i][j], ns->ed.stn, &incell_left);
        // Get the Kernel in the right cell
        real Kright = compute_center_p_right_22(ns->ed.sdED, ccenter, cdelta, dim, 1.0, ns->ed.vepl.dpKernel[i][j], ns->ed.stn, &incell_right);
