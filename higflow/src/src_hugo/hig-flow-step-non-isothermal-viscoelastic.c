...
#include "hig-flow-step-non-isothermal-viscoelastic.h"
...
// *******************************************************************
void higflow_explicit_euler_constitutive_equation_non_isothermal_flows(higflow_solver *ns)
{
    if ((ns->contr.flowtype == 3))
    {
        // Get the cosntants
        ...
        for (it = sd_get_domain_celliterator(sdp); !higcit_isfinished(it); higcit_nextcell(it))
        {
            ...
            real Temperature = compute_value_at_point(ns->ed.nif.sdT, ccenter, ccenter, 1.0, ns->ed.nif.dpT, ns->ed.stn);
            //DEBUG_INSPECT(Temperature,%lf);
            ...
            case 0:
                ...
                fTemp = function_to_scale_property_with_temperature_arrhenius(Temperature, T0, T1, alphaT);
                //fTemp = function_to_scale_property_with_temperature_arrhenius(0.0, T0, T1, alphaT);
                break;
            case 1:
                // Function f(T): Modified-Arrhenius
                fTemp = function_to_scale_property_with_temperature_modified_arrhenius(Temperature, T0, T1, alphaT);
                break;
            case 2:
                // Function f(T): Vogel-Tamman-Fulcher (VTF)
                fTemp = function_to_scale_property_with_temperature_VTF(Temperature, T0, T1, A0, B0);
                break;
            case 3:
                // Function f(T): Williams-Landel-Ferry
                fTemp = function_to_scale_property_with_temperature_WLF(Temperature, T0, T1, c1, c2);
                break;
            case 4:
            ...
            //DEBUG_INSPECT(fTemp,%lf);
            // Calculate kernel matrix with f(T) MMV = MM/f(T) (Do a loop here)
            real MMV[DIM][DIM];
            for (int i = 0; i < DIM; i++)
            {
                for (int j = 0; j < DIM; j++)
                {
                    MMV[i][j] = MM[i][j] / fTemp;
                }
            }
            //DEBUG_INSPECT(MM[0][0],%lf);
            //DEBUG_INSPECT(MMV[0][0],%lf);
            ...
            for (int i = 0; i < DIM; i++)
            {
                for (int j = i; j < DIM; j++)
                {
                    ...
                    // Calculate max and min K
                    // if (kernel > Kmax[i][j]) Kmax[i][j] = kernel;
                    // if (kernel < Kmin[i][j]) Kmin[i][j] = kernel;
                }
            }
        }
        // Destroy the iterator
...
// *******************************************************************
void higflow_implicit_euler_constitutive_equation_non_isothermal_flows(higflow_solver *ns)
{
    if ((ns->contr.flowtype == 3) || (ns->contr.flowtype == 2))
    {
        // Get the cosntants
...
// *******************************************************************
void higflow_explicit_euler_intermediate_velocity_viscoelastic_non_isothermal_flows(higflow_solver *ns, distributed_property *dpu[DIM], distributed_property *dpustar[DIM])
{
    ...
    for (int dim = 0; dim < DIM; dim++)
    {
        ...
        for (fit = sfd_get_domain_facetiterator(sfdu[dim]); !higfit_isfinished(fit); higfit_nextfacet(fit))
        {
            ...
            higflow_computational_cell_bousinessq_term_momentum_equation(ns, flid, fcenter, ns->ed.nif.dpT);
            // Right hand side equation
            real rhs = 0.0;
...
// *******************************************************************
void higflow_explicit_runge_kutta_2_intermediate_velocity_viscoelastic_non_isothermal_flows(higflow_solver *ns)
{
    // Calculate the auxiliar velocity by the explicit euler method
    higflow_explicit_euler_intermediate_velocity_viscoelastic_non_isothermal_flows(ns, ns->dpu, ns->dpuaux);
    // Calculate the star velocity by the explicit euler method
    higflow_explicit_euler_intermediate_velocity_viscoelastic_non_isothermal_flows(ns, ns->dpuaux, ns->dpustar);
...
// *******************************************************************
void higflow_explicit_runge_kutta_3_intermediate_velocity_viscoelastic_non_isothermal_flows(higflow_solver *ns)
{
    // Calculate the auxiliar velocity by the explicit euler method
    higflow_explicit_euler_intermediate_velocity_viscoelastic_non_isothermal_flows(ns, ns->dpu, ns->dpuaux);
...
// *******************************************************************
void higflow_semi_implicit_euler_intermediate_velocity_viscoelastic_non_isothermal_flows(higflow_solver *ns)
{
    // Get the facet iterator
    higfit_facetiterator *fit;
    // Get the local domain for cell
    ...
    for (int dim = 0; dim < DIM; dim++)
    {
        ...
        for (fit = sfd_get_domain_facetiterator(sfdu[dim]); !higfit_isfinished(fit); higfit_nextfacet(fit))
        {
            ...
            for (int dim2 = 0; dim2 < DIM; dim2++)
            {
                ...
                real w = -(ns->par.dt) * (1.0 * (ns->cc.FT) * (ns->ed.ve.par.beta) + 1.0 * (1.0 - ns->ed.ve.par.beta)) / (ns->par.Re * fdelta[dim2] * fdelta[dim2]);
                alpha -= 2.0 * w;
                Point p;
...
// *******************************************************************
void higflow_semi_implicit_crank_nicolson_intermediate_velocity_viscoelastic_non_isothermal_flows(higflow_solver *ns)
{
    // Get the facet iterator
    higfit_facetiterator *fit;
    // Get the local domain for cell
    ...
    for (int dim = 0; dim < DIM; dim++)
    {
        ...
        for (fit = sfd_get_domain_facetiterator(sfdu[dim]); !higfit_isfinished(fit); higfit_nextfacet(fit))
        {
            ...
            {
                // Stencil weight update
                real w = -0.5 * (ns->par.dt) * (1.0 * (ns->cc.FT) * (ns->ed.ve.par.beta) + 1.0 * (1.0 - ns->ed.ve.par.beta)) / (ns->par.Re * fdelta[dim2] * fdelta[dim2]);
                alpha -= 2.0 * w;
                Point p;
                POINT_ASSIGN(p, fcenter);
                // Stencil point update: right point
...
// *******************************************************************
void higflow_semi_implicit_bdf2_intermediate_velocity_viscoelastic_non_isothermal_flows(higflow_solver *ns, distributed_property *dpu[DIM], distributed_property *dpustar[DIM])
{
    ...
    for (int dim = 0; dim < DIM; dim++)
    {
        ...
        for (fit = sfd_get_domain_facetiterator(sfdu[dim]); !higfit_isfinished(fit); higfit_nextfacet(fit))
        {
            ...
            {
                // Stencil weight update
                real w = -0.25 * (ns->par.dt) * (1.0 * (ns->cc.FT) * (ns->ed.ve.par.beta) + 1.0 * (1.0 - ns->ed.ve.par.beta)) / (ns->par.Re * fdelta[dim2] * fdelta[dim2]);
    ...
    for (int dim = 0; dim < DIM; dim++)
    {
        ...
        for (fit = sfd_get_domain_facetiterator(sfdu[dim]); !higfit_isfinished(fit); higfit_nextfacet(fit))
        {
            ...
            {
                // Stencil weight update
                real w = -1.0 / 3.0 * (ns->par.dt) * (1.0 * (ns->cc.FT) * (ns->ed.ve.par.beta) + 1.0 * (1.0 - ns->ed.ve.par.beta)) / (ns->par.Re * fdelta[dim2] * fdelta[dim2]);
                alpha -= 2.0 * w;
                Point p;
                POINT_ASSIGN(p, fcenter);
                // Stencil point update: right point
...
void hig_flow_calculate_m_gptt(higflow_solver *ns, real tr, real lambda[DIM], real M[DIM][DIM], real R[DIM][DIM], real M_aux[DIM][DIM], real tol)
{
    ...
    for (int i = 0; i < DIM; i++)
    {
        ...
        // Exponêncial
        real mittreal;
        real argexp = ns->ed.ve.par.epsilon * ns->par.Re * ns->ed.ve.par.De * tr / (1.0 - ns->ed.ve.par.beta);
        mittreal = exp(argexp);
        // real argexp = ns->ed.ve.par.beta-1.0;
        // mittreal = (ns->ed.ve.par.epsilon*ns->par.Re*ns->ed.ve.par.De*tr/(1.0-ns->ed.ve.par.beta))*exp(argexp);
        /*if(gama1==0)
         printf("Gamma1=0");
        else
        printf("===> gamma1: %lf=\n",gama1);
...
// Calculate f(T) using the Arrhenius model
real function_to_scale_property_with_temperature_arrhenius(real TempD, real T0, real T1, real alphaT)
{
    real Temp, F1T, FT;
    // Calculation of dimensional temperature
    //Temp = 500;
    //Temp = TempD * (T1 - T0) + T0;
    Temp = TempD * (T0 - T1) + T1;
    F1T = alphaT * ((1.0 / Temp) - (1.0 / T0));
    // Calculation of FT
    FT = exp(F1T);
    return FT;
}

// Calculate f(T) using the Modified-Arrhenius model
real function_to_scale_property_with_temperature_modified_arrhenius(real TempD, real T0, real T1, real alphaT)
{
    real Temp, F1T, FT;
    // Calculation of dimensional temperature
    Temp = TempD * (T1 - T0) + T0;
    F1T = -1.0 * (alphaT * (Temp - T0));
    // Calculation of FT
    FT = exp(F1T);
    return FT;
}

// Calculate f(T) using the Vogel-Tamman-Fulcher (VTF) model
real function_to_scale_property_with_temperature_VTF(real TempD, real T0, real T1, real A0, real B0)
{
    real Temp, F1T, FT;
    // Calculation of dimensional temperature
    Temp = TempD * (T1 - T0) + T0;
...
// Calculate f(T) using the Williams-Landel-Ferry (WLF) model
real function_to_scale_property_with_temperature_WLF(real TempD, real T0, real T1, real c1, real c2)
{
    real Temp, F1T, FT;
    // Calculation of dimensional temperature
    Temp = TempD * (T1 - T0) + T0;
    F1T = -1.0 * ((c1 * (Temp - T0)) / (c2 + (Temp - T0)));
    // Calculation of FT
    FT = pow(10.0, F1T);
    return FT;
}
...
void hig_flow_derivative_temperature_at_center_cell(higflow_solver *ns, Point ccenter, Point cdelta, real Tcenter, real dTdx[DIM])
{
    for (int dim = 0; dim < DIM; dim++)
    {
        int incell_left, incell_right;
        // Get the concentration in the left cell
        real Tleft = compute_center_p_left_22(ns->ed.nif.sdT, ccenter, cdelta, dim, 1.0, ns->ed.nif.dpT, ns->ed.stn, &incell_left);
        // Get the concentration in the right cell
        real Tright = compute_center_p_right_22(ns->ed.nif.sdT, ccenter, cdelta, dim, 1.0, ns->ed.nif.dpT, ns->ed.stn, &incell_left);
        // Compute the concentration derivative
        if ((incell_left == 1) && (incell_right == 1))
        {
            dTdx[dim] = compute_dpdx_at_point(cdelta, dim, 1.0, Tleft, Tright);
        }
        else if (incell_right == 1)
        {
            dTdx[dim] = compute_dpdxr_at_point(cdelta, dim, 1.0, Tcenter, Tright);
        }
        else
        {
            dTdx[dim] = compute_dpdxl_at_point(cdelta, dim, 1.0, Tleft, Tcenter);
        }
    }
}

// *******************************************************************
// Calculate convective energy equation term CUBISTA
// *******************************************************************
real higflow_convective_temperature_term_cubista_non_isothermal_flows(higflow_solver *ns, distributed_property *dpu, distributed_property *dpT, sim_domain *sdp, sim_stencil *stn, real T, Point ccenter, Point cdelta, int dim)
{
    real vbar[DIM], dKdx[dim], kr, krr, kl, kll, kc, a, b, c, d, e, tol, fi, conv1, conv2;
    a = 1.7500;
    b = 0.3750;
    ...
    kc = T;
    // Get the low, high, lowlow, highhigh component kernel at center cell
    kl = compute_center_p_left_22(sdp, ccenter, cdelta, dim, 1.0, dpT, stn, &incell_l);
    kr = compute_center_p_right_22(sdp, ccenter, cdelta, dim, 1.0, dpT, stn, &incell_r);
    kll = compute_center_p_left_22(sdp, ccenter, cdelta, dim, 2.0, dpT, stn, &incell_ll);
    krr = compute_center_p_right_22(sdp, ccenter, cdelta, dim, 2.0, dpT, stn, &incell_rr);
    // Get the velocity  v1bar(i+1/2,j) in the facet center
    vbar[dim] = compute_facet_u_right(ns->sfdu[dim], ccenter, cdelta, dim, 0.5, ns->dpu[dim], ns->stn, &infacet);
    if (vbar[dim] > 0.0)
    {
...
// Computing the Bousinessq source term
void higflow_compute_bousinessq_term(higflow_solver *ns)
{
    real etamax = -1.0e16;
    real etamin = 1.0e16;
    real qmax = -1.0e16;
    real qmin = 1.0e16;
    real Re = ns->par.Re;
    real Pr = ns->ed.nif.par.Pr;
    real Br = ns->ed.nif.par.Br;
    ...
    if (ns->contr.modelflowtype == 5)
    {
        ...
        for (it = sd_get_domain_celliterator(sdp); !higcit_isfinished(it); higcit_nextcell(it))
        {
            ...
            Point cdelta;
            hig_get_delta(c, cdelta);
            // Compute the temperature value at point
            real Temperature = compute_value_at_point(ns->ed.nif.sdT, ccenter, ccenter, 1.0, ns->ed.nif.dpT, ns->ed.stn);
            real BST = (Ga - Gr * (Temperature)) / (pow(Re, 2.0));
            // Calculate the energy source term
            // real EST = ns->ed.nif.get_energy_source_term(ccenter, ns->par.t, Temperature, T0, T1, Pr, Br, Gr, Ga);
            ...
            dp_set_value(ns->ed.nif.dpEF, clid, BST);
        }
...
// Solving the energy equation for non-isothermal flows
void higflow_explicit_euler_energy_equation(higflow_solver *ns)
{
    if (ns->contr.modelflowtype == 5)
    {
        real Pr = ns->ed.nif.par.Pr;
        ...
        // Get the cosntants
        // Get the local sub-domain for the cells
        sim_domain *sdT = psd_get_local_domain(ns->ed.nif.psdT);
        // Get the local sub-domain for the facets
        sim_facet_domain *sfdu[DIM];
        for (int i = 0; i < DIM; i++)
        {
        ...
        // Get the map for the domain properties
        mp_mapper *mp = sd_get_domain_mapper(sdT);
        // Loop for each cell
        higcit_celliterator *it;
        for (it = sd_get_domain_celliterator(sdT); !higcit_isfinished(it); higcit_nextcell(it))
        {
            // Get the cell
            hig_cell *c = higcit_getcell(it);
            // Get the cell identifier
            ...
            if (TD < TDmin)
                TDmin = TD;
            // Get the velocity at cell center
            real u[DIM], dTdx[DIM], Temperature;
            hig_flow_velocity_at_center_cell(ns, ccenter, cdelta, u);
            // Compute the temperature value at point
            Temperature = compute_value_at_point(ns->ed.nif.sdT, ccenter, ccenter, 1.0, ns->ed.nif.dpT, ns->ed.stn);
            //DEBUG_INSPECT(Temperature,%lf);
            ...
            case 0:
                ...
                hig_flow_derivative_temperature_at_center_cell(ns, ccenter, cdelta, Temperature, dTdx);
                for (int dim = 0; dim < DIM; dim++)
                {
                    // Set the computational cell
                    higflow_computational_cell_energy_equation(ns, sdT, clid, ccenter, cdelta, dim, ns->ed.nif.dpT);
                    // Compute convective inonic term in rhs
                    rhs -= u[dim] * dTdx[dim];
                    // Compute the diffusive energy term rhs
                    //rhs += higflow_difussion_energy_equation_term(ns);
                }
                break;
            // CUBISTA scheme
            case 1:
                for (int dim = 0; dim < DIM; dim++)
                {
                    ...
                    rhs -= higflow_convective_temperature_term_cubista_non_isothermal_flows(ns, ns->dpu[dim], ns->ed.nif.dpT, ns->ed.nif.sdT, ns->ed.stn, Temperature, ccenter, cdelta, dim);
            ...
            rhs += higflow_difussion_energy_equation_term(ns);
            //real rhs0 = higflow_difussion_energy_equation_term(ns);
            //DEBUG_INSPECT(rhs0,%lf);
            // Addition of the energy source term
            real EST = ns->ed.nif.get_energy_source_term(ccenter, ns->par.t, Temperature, T0, T1, Pr, Br, Gr, Ga);
            // Addition of the energy source term
            rhs += EST;
            // Viscous dissipation term activated
            //real RHSTest;
            if (ns->ed.nif.contr.visdissipation == 1)
            {
                //RHSTest = (Br/Pr)*TD;
                rhs += (Br / Pr) * TD;
                //rhs += (Br / Pr) * TD;
            }
            //DEBUG_INSPECT(EST,%lf);
            // Compute the final value step time
            real newTemperature = Temperature + ns->par.dt * rhs;
            //Check to see min and max structural parameter values
            if (newTemperature > Tempmax) Tempmax = newTemperature;
            if (newTemperature < Tempmin) Tempmin = newTemperature;
            // Set property value
            dp_set_value(ns->ed.nif.dpT, clid, newTemperature);
        }
        //Printing the min and max structural parameter values
        printf("===> Tempmin = %lf <===> Tempmax = %lf <===\n", Tempmin, Tempmax);
        // Destroy the iterator
        higcit_destroy(it);
...
// Solving the energy equation for non-isothermal flows using the implicit method
void higflow_implicit_euler_energy_equation(higflow_solver *ns)
{
    if (ns->contr.modelflowtype == 5)
    {
        real Pr = ns->ed.nif.par.Pr;
        ...
        for (it = sd_get_domain_celliterator(sdT); !higcit_isfinished(it); higcit_nextcell(it))
        {
            ...
            case 0:
                ...
                for (int dim = 0; dim < DIM; dim++)
                {
                    ...
                    rhs -= u[dim] * dTdx[dim];
                    // Compute the diffusive energy term rhs
                    //rhs += higflow_implicit_difussion_energy_equation_term(ns);
                }
                break;
            // CUBISTA scheme
            case 1:
            ...
            {
                // Stencil weight update
                real w = -(ns->par.dt) * (ns->cc.KTR + ns->cc.KTL) / ((ns->ed.nif.par.Pr) * (ns->par.Re) * cdelta[dim2] * cdelta[dim2]);
                alpha -= 2.0 * w;
                Point p;
                POINT_ASSIGN(p, ccenter);
                // Stencil point update: right point
                p[dim2] = ccenter[dim2] + cdelta[dim2];
                wr = (ns->par.dt) * (ns->cc.KTR) / ((ns->ed.nif.par.Pr) * (ns->par.Re) * cdelta[dim2] * cdelta[dim2]);
                sd_get_stencil(sdT, ccenter, p, wr, ns->ed.stn);
                // Stencil point update: left point
                p[dim2] = ccenter[dim2] - cdelta[dim2];
                wl = (ns->par.dt) * (ns->cc.KTL) / ((ns->ed.nif.par.Pr) * (ns->par.Re) * cdelta[dim2] * cdelta[dim2]);
                sd_get_stencil(sdT, ccenter, p, wl, ns->ed.stn);
            }
            alpha = 1.0 + alpha;
            // Get the stencil
            sd_get_stencil(sdT, ccenter, ccenter, alpha, ns->ed.stn);
            // Get the index of the stencil
            int *ids = psd_stn_get_gids(ns->ed.nif.psdT, ns->ed.stn);
            // Get the value of the stencil
            real *vals = stn_get_vals(ns->ed.stn);
            // Get the number of elements of the stencil
            int numelems = stn_get_numelems(ns->ed.stn);
            int cgid = psd_get_global_id(ns->ed.nif.psdT, c);
            // Set the right side of solver linear system
            slv_set_bi(ns->ed.nif.slvT, cgid, stn_get_rhs(ns->ed.stn));
            // Set the line of matrix of the solver linear system
            slv_set_Ai(ns->ed.nif.slvT, cgid, numelems, ids, vals);
        }

        // Destroy the iterator
        higcit_destroy(it);
        // Assemble the solver
        slv_assemble(ns->ed.nif.slvT);
        // Solve the linear system
        slv_solve(ns->ed.nif.slvT);
        // Load property from solver
        dp_slv_load_from_solver(ns->ed.nif.dpT, ns->ed.nif.slvT);
...
void higflow_solver_step_non_isothermal_flows(higflow_solver *ns)
{
    ...
    // Compute the coefficient of the Bousinessq approximation
    if (ns->ed.nif.contr.bousinessq == 1)
    {
        higflow_compute_bousinessq_term(ns);
    }
    ...
    case 0:
        ...
        higflow_explicit_euler_intermediate_velocity_viscoelastic_non_isothermal_flows(ns, ns->dpu, ns->dpustar);
        break;
    case 1:
        // Explicit RK2 method
        higflow_explicit_runge_kutta_2_intermediate_velocity_viscoelastic_non_isothermal_flows(ns);
        break;
    case 2:
        // Explicit RK3 method
        higflow_explicit_runge_kutta_3_intermediate_velocity_viscoelastic_non_isothermal_flows(ns);
        break;
    case 3:
        // Semi-Implicit Euler Method
        higflow_semi_implicit_euler_intermediate_velocity_viscoelastic_non_isothermal_flows(ns);
        break;
    case 4:
        // Semi-Implicit Crank-Nicolson Method
        higflow_semi_implicit_crank_nicolson_intermediate_velocity_viscoelastic_non_isothermal_flows(ns);
        break;
    case 5:
        // Semi-Implicit Crank-Nicolson Method
        higflow_semi_implicit_bdf2_intermediate_velocity_viscoelastic_non_isothermal_flows(ns, ns->dpu, ns->dpustar);
        break;
    }
    ...
    case 0:
        // Explicit method
        higflow_explicit_euler_constitutive_equation_non_isothermal_flows(ns);
        break;
    case 1:
        // Implicit method
        higflow_implicit_euler_constitutive_equation_non_isothermal_flows(ns);
        break;
    }
    // Computing the Polymeric Tensor
    ...
    case 0:
        // Explicit method
        higflow_explicit_euler_energy_equation(ns);
        break;
    case 1:
        // Implicit method
        higflow_implicit_euler_energy_equation(ns);
        break;
    }
}
