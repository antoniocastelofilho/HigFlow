 ...
 *u(x,y,0) = f(x,y)
 *v1, v2 > 0
 *Using Lax-Friedrichs to solve it
 */
...
void set_dirichlet_boundary_conditions(sim_domain *sd, hig_cell *root) {
    ...
    sd_create_boundary(root, temp, NEUMANN);
    for(int t =0; t < 2; t++) { //iterating types of bc
        for(int i =0; i < 4; i++) { //iterating sides
            sim_boundary *sb = sd_get_bc(temp, t, i); /*We get the BCs*/
            hig_cell *sbc = sb_get_higtree(sb);
            hig_cell *test = hig_get_cell_with_point(sbc, lo);
            if((test != NULL && t == 0) || (test == NULL && t == 1)) { /*If low-point and DIRICHLET or if top/right and NEUMANN (outflow), set value to 0*/
...
int main(int argc, char *argv[]) {
    ...
    sd_set_interpolator_order(localdomain, 1); /*If it's bigger than 1, the method becomes unstable for some reason*/
    ...
    for(int t = 0; t < total_iterations; t++) {
        ...
        for(it = sd_get_domain_celliterator(localdomain); !higcit_isfinished(it); higcit_nextcell(it)) {
            ...
            int cgid = mp_lookup(m, hig_get_cid(c));
            real uright = sd_dp_interpolate(localdomain, dpu_last, ccenter, right, stn);
            real uleft = sd_dp_interpolate(localdomain, dpu_last, ccenter, left, stn);
            real utop = sd_dp_interpolate(localdomain, dpu_last, ccenter, top, stn);
            ...
            real next_u = (uright+uleft+utop+ubottom)/4 -((v1*dt)/(2*dx))*(uright-uleft) -((v2*dt)/(2*dy))*(utop-ubottom);
