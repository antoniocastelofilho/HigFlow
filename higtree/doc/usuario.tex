...
Note que neste tutorial, estamos lendo a malha a partir de um arquivo \texttt{.amr} (Adaptive Mesh Refinement). Na seção \ref{sec:amr}, será explicado de forma detalhada como esses arquivos são usados.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Para partir para um próximo passo temporal, antes temos que usar a função \texttt{dp\_sync(dpu\_last)} para que, se estivermos rodando o código em paralelo, atualizamos as fringes.
Foram usados dois métodos distintos para lidar com as derivadas espaciais desta equação: Lax-Friedrichs e Upwind, respectivamente nos arquivos \textit{tutorial09-lax-2d.c} e \textit{tutorial09-upwind-2d.c}. Para Lax, é necessário definir uma BC de outflow nas bordas direitas e superior, dado que o método usa pontos downwind.

\begin{verbatim}    
    for(int t = 0; t < total_iterations; t++) {
        dp_sync(dpu_last);
        for(it = sd_get_domain_celliterator(localdomain); 
                        !higcit_isfinished(it); higcit_nextcell(it)) {
            hig_cell *c = higcit_getcell(it);
        ...
        dp_copy_values(dpu_last, dpu_now);
        time += dt;
        higcit_destroy(it);
    }
\end{verbatim}

Utilizando o código com o upwind implementado para comentar, vemos cinco funções principais utilizadas para trabalhar com as \textit{distributed properties}. A primeira, já comentada é a \texttt{dp\_sync()}. A função \texttt{sd\_dp\_interpolate()} calcula a interpolação da \textit{distributed property} no ponto definidio, utilizando os métodos de interpolação já discutidos (existe a variante \texttt{sfd\_dp\_interpolate()} para domínios de faces). \texttt{dp\_get\_value()} simplesmente retorna o valor em um determinado id e \texttt{dp\_set\_value()} define um valor para o id. Por fim, \texttt{dp\_copy\_values} copia os valores de uma \textit{distributed property} para outra.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Arquivos .amr}
\label{sec:amr}
Arquivos \texttt{.amr} armazenam a estrutura de uma malha, ou seja, consegue representar um domínio ou até uma condição de contorno. Quanto a sua estrutura, ela possui os seguintes blocos:

Na primeira linha, temos as coordenadas dos pontos $\textit{high} = (x_2, y_2)$ e $\textit{low} = (x_1, y_1)$ do retângulo (paralelepípedo) que compõe a célula raíz.
\begin{verbatim}
  x1  x2   y1  y2
 -1.0 1.0 -1.0 1.0
\end{verbatim}
No caso tridimensional, colocamos \texttt{ x1 x2 y1 y2 z1 z2}.

Na segunda linha, colocamos o número de níveis de Refinamento
\begin{verbatim}
 2
\end{verbatim}

Nas próximas linhas, descrevemos os níveis de refinamento. Cada nível possui um número de \textit{patches} e o tamanho de cada célula após o refinamento. Cada \textit{patch}, por sua vez, define a célula inicial a se aplicar o refinamento e a quantidade de células, a partir do ponto inicial, a serem refinadas.

\begin{verbatim}
primeiro nivel
delta_x delta_y  num_patches
0.05     0.05       1
patch 1
x_ini y_ini  qnt_x qnt_y
1       1     20    20
...
A posição da célula inicial e a quantidade de células para se aplicar o patch são baseadas na malha uniforme que seria feita ao dividir toda a raíz pelo delta desse nível. No exemplo acima, no primeiro nível temos $(dx, dy) = (0.05, 0.05)$. Como o domínio tem tamanho 2 para as duas direções, temos que a quantidade de células é $(\frac{2}{0.05}, \frac{2}{0.05}) = (40, 40)$. Na linha do primeiro patch, \texttt{1 1 40 40} estamos falando para iniciar na primeira célula e andar 40 células (com espaçamento de $0.05$) para ambas as direções, percorrendo todo o domínio. Note que a quantidade de células inclui a célula inicial.

No segundo nível, temos no total $(80,80)$ células. Fazemos dois patches que são um quadrado de $1/4$ do domínio.

Por fim, obtemos o seguinte \texttt{.amr} para essa malha:
\begin{verbatim}
-1.0 1.0 -1.0 1.0
2
0.05 0.05 1
...
Que gera a malha da figura \ref{fig:malha_amr}.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{figuras/malha_exemplo.png}
    \caption{Malha gerada pelo .amr}
    \label{fig:malha_amr}
\end{figure}

Para usar arquivos \texttt{.amr} na biblioteca higtree, o módulo \textit{higtree-io} proporciona duas maneiras para a leitura desses arquivos. A primeira utiliza a função \texttt{higio\_read\_from\_amrDIMd()} e retorna imediatamente a \texttt{hig\_cell} correspondente. A segunta utiliza a função \texttt{higio\_read\_amr\_info()}, que lê o arquivo e o transforma na estrutura \texttt{higio\_amr\_info}, que pode ser utilizada em outras funções do pacote de I/O da higtree.
% \section{Parte I}
