...
static solver *
setup_p_solver(psim_domain *psdp, psim_facet_domain *psfdu[DIM], distributed_property *dpp, distributed_property *dpustar[DIM], real dt, real Re, sim_stencil *stn) {
...
int main (int argc, char *argv[])
{
    ...
    int total_numprobes = atoi(argv[1]); argv++;
    ...
    char *results = argv[1]; argv++;

    bool use_xdmf;
    if(strcasecmp(argv[1], "VTK") == 0) {
	use_xdmf = false;
    } else if(strcasecmp(argv[1], "XDMF") == 0) {
	use_xdmf = true;
    } else {
    ...
    /* Get the bcs assigned to this process. */
    {
		...
		unsigned group_count = lb_get_num_local_trees_in_group(part_ctx, g);
		for(unsigned i = 0; i < group_count; ++i) {
			bctrees[numbcs] = lb_get_local_tree_in_group(part_ctx, i, g);
	    ...
	    computeBCU(psfdu, numbcs, bctrees, l_ubctypes, l_ubcvalues, l_vbctypes, l_vbcvalues);
    }
	...
	unsigned prid = g - total_numbcs - 1;

	unsigned group_count = lb_get_num_local_trees_in_group(part_ctx, g);
	for(unsigned i = 0; i < group_count; ++i) {
		probe[numprobes] = lb_get_local_tree_in_group(part_ctx, i, g);
    ...
    ///* Print the pressure */
    char vtkname[1024];
    writePBC(psdp, results, "bc", myrank);
    ...
    if(use_xdmf) {
	xdmf_out = xdmf_init(results, sdp);
    ...
    for(int step = 0; step < numsteps; step++) {
        ...
        if (step % perstep == 0) {
            ...
            printResults(psdp, psfdu, numprobes, probe, l_prop, factor, dpp, dpu);

            /* Printing the velocities to visualize */
	    if(use_xdmf) {
		    xdmf_write_timestep(xdmf_out, numframe, numframe);
	    } else {
	            writeframe(psdp, psfdu[0], dpp, dpu[0], results, "u", myrank, numframe);
        	    writeframe(psdp, psfdu[1], dpp, dpu[1], results, "v", myrank, numframe);
	    }
