...
void computeBCP(psim_domain *psd, int numbcs, char *bcfilenames[], int pbctypes[], real pbcvalues[]);
void computeBCU(psim_facet_domain *psfdu[DIM], int numbcs, char *bcfilenames[], int ubctypes[], real ubcvalues[], int vbctypes[], real vbcvalues[]);
...
void computeBCP(psim_domain *psd, int numbcs, char *bcfilenames[], int pbctypes[], real pbcvalues[]) {
	...
	for(int h = 0; h < numbcs; h++) {
		...
		bcg[h] = higio_read_from_amr2d(fd);
	}
	for(int h = 0; h < numbcs; h++) {
...
//	DEBUG_DIFF_TIME;
	...
	int numbcs = atoi(argv[1]);
	argv++;
	char *amrBCfilename[numbcs];
	int pbctypes[numbcs];
	int ubctypes[numbcs];
	int vbctypes[numbcs];
	real pbcvalues[numbcs];
	real ubcvalues[numbcs];
	real vbcvalues[numbcs];
	for(int h = 0; h < numbcs; h++) {
	...
	int numprobes = atoi(argv[1]); argv++;
	hig_cell *probe[numprobes];
	int prop[numprobes];
	char *probefilename[numprobes];
	for(int h = 0; h < numprobes; h++) {
		probefilename[h] = argv[1]; argv++;
		prop[h] = atoi(argv[1]); argv++;
		FILE *fd = fopen(probefilename[h], "r");
		probe[h] = higio_read_from_amr2d(fd);
	...
	int extraload = atoi(argv[1]); argv++;
	...
	for(int h = myrank; h < numhigs; h += ntasks) {
		...
		hig_cell *tree = higio_read_from_amr2d(fd);

		/* Closing the AMR format file */
		fclose(fd);

		lb_add_input_tree(lb, tree, true, 0);
	}

	lb_calc_partition(lb, pg);

	/* Create retrieve partitioned domain. */
	{
		unsigned numtrees = lb_get_num_local_trees(lb);
		for(unsigned i = 0; i < numtrees; ++i) {
			hig_cell *root =
				lb_get_local_tree(lb, i, NULL);
	...
	computeBCP(psdp, numbcs, amrBCfilename, pbctypes, pbcvalues);
...
//	DEBUG_DIFF_TIME
	...
	//computeBCU(psfdu, mi[0]);
	computeBCU(psfdu, numbcs, amrBCfilename, ubctypes, ubcvalues, vbctypes, vbcvalues);
...
//			DEBUG_DIFF_TIME;
	...
	printf("total time (rank %d) = %lf secs\n", myrank, GET_NSEC_CLOCK(total)/(1.0e9));
	printf("ns time (rank %d) = %lf secs\n", myrank, GET_NSEC_CLOCK(ns)/(1.0e9));
	printf("poisson time (rank %d) = %lf secs\n", myrank, GET_NSEC_CLOCK(poisson)/(1.0e9));
	printf("extraload time (rank %d) = %lf secs\n", myrank, GET_NSEC_CLOCK(extraload)/(1.0e9));
