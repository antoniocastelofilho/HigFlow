...
void initializeF(psim_domain *psdp, distributed_property *dpF) ;
...
void compute_p(psim_domain *psdp, distributed_property *dpp, distributed_property *dpF, sim_stencil *stn) ;
...
void updateP(psim_domain *psdp, distributed_property *dpp, distributed_property *dpF, sim_stencil *stn) ;

void writePBC(psim_domain *psdp, const char *basefilename, const char *propertyname, int rank) ;

void writePres(psim_domain *psdp, distributed_property *dpp, const char *basefilename, const char *propertyname, int rank) ;
...
void initializeF(psim_domain *psdF, distributed_property *dpF) {
        /* Setting the initial value for the pressure into the cavity */
	...
	higcit_celliterator *it;
        ...
        /* Getting the local domain */
	sim_domain *sdF = psd_get_local_domain(psdF);

        /* Getting the Mapper for the local domain */
	mp_mapper *m = sd_get_domain_mapper(sdF);

        /* Calculating the velocity at the facet cell */
	Point l, h, delta;
	hig_cell *dom = sd_get_higtree(sdF, 0);
	hig_get_lowpoint(dom, l);
	hig_get_highpoint(dom, h);
	hig_get_delta(dom, delta);

        /* Traversing the cells of local domain */
	for(it = sd_get_domain_celliterator(sdF); !higcit_isfinished(it); higcit_nextcell(it)) {
		...
		POINT_DIV(x, x, delta);
                real val;
		val = analyticF(3.141593*x[0], 3.141593*x[1]);

                /* Set the value 0.0 for the pressure in this cell */
		dp_set_value(dpF, cgid, val);
...
void compute_p(psim_domain *psdp, distributed_property *dpp, distributed_property *dpF, sim_stencil *stn) {
	...
	for(it = sd_get_domain_celliterator(sdp); !higcit_isfinished(it); higcit_nextcell(it)) {
		...
		hig_get_delta(c, cdelta);
                ...
                /* Getting the force at the cell */
                real val;
		val = sd_dp_interpolate(sdp, dpF, ccenter, ccenter, stn);
...
int main (int argc, char *argv[])
{
	...
	distributed_property * dpF = psd_create_property(psdp);
...
//	writePBC(psdp, "vtkpoissonutilde/bc", "p", myrank);
	writePres(psdp, dpp, "vtkpoissonforce/mesh", "p", myrank);
